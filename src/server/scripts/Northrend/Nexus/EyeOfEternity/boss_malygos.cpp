...
static LocationsXYZ GOPositions[]=
{
        ...
        void DoVortex(uint8 phase)
        {
            ...
            {    
                ...
                for(Map::PlayerList::const_iterator itr = lPlayers.begin(); itr != lPlayers.end(); ++itr)
                {
                    ...
                    itr->getSource()->NearTeleportTo(VortexLoc[0].x, VortexLoc[0].y, VORTEX_Z + 17, 0);
                }
...
public:
    ...
    {
        if (pGO->GetInstanceScript()->GetData(TYPE_MALYGOS) == NOT_STARTED)
        {
            pGO->GetInstanceScript()->SetData(TYPE_MALYGOS,SPECIAL);
        }
        return true;
    }
};
...
class go_malygos_portal : public GameObjectScript
{
public:
    go_malygos_portal() : GameObjectScript("go_malygos_portal") { }
